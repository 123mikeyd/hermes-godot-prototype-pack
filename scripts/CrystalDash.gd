...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(20, 35), "Crystal Dash  |  Score %d/10  |  Time %.1f  |  WASD move, R restart, Esc menu" % [score, max(time_left, 0.0)], HORIZONTAL_ALIGNMENT_LEFT, -1, 20, Color(0.9, 0.95, 1.0))
	if over:
